const PoziviAjax = (() => {
    ...
    function impl_getTop5Nekretnina(lokacija, fnCallback) {
        const url = `/nekretnine/top5?lokacija=${lokacija}`;
        ajaxRequest('GET', url, null, (error, data) => {
            if (error) {
                fnCallback(error, null);
            } else {
                try {
                    const top5Properties = JSON.parse(data);
                    fnCallback(null, top5Properties);
                } catch (parseError) {
                    fnCallback(parseError, null);
                }
            }
    ...
    function impl_getNekretnina(nekretnina_id, fnCallback) {
        ...
        ajaxRequest('GET', url, null, (error, data) => {
            if (error) {
                ...
                try {
                    const propertyDetails = JSON.parse(data);
                    fnCallback(null, propertyDetails);
                } catch (parseError) {
                    fnCallback(parseError, null);
                }
            }
    ...
    function impl_getNextUpiti(nekretnina_id, page, fnCallback) {
        ...
        ajaxRequest('GET', url, null, (error, data) => {
            if (error) {
                ...
                try {
                    const nextQueries = JSON.parse(data);
                    fnCallback(null, nextQueries);
                } catch (parseError) {
                    fnCallback(parseError, null);
                }
            }
